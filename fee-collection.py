...
def signup():
    ...
    def submit_signup():
        ...
        pending_fee = float(signup_fee_var.get())

        if username and password and pending_fee >= 0:
            insert_student(conn, (username, password, pending_fee))
            messagebox.showinfo("Signup Successful", "Signup successful! You can now login.")
    ...
    signup_fee_var = StringVar()

    Label(signup_window, text="Username").grid(row=0, column=0, padx=10, pady=10)
    Entry(signup_window, textvariable=signup_username_var).grid(row=0, column=1, padx=10, pady=10)
    
    Label(signup_window, text="Password").grid(row=1, column=0, padx=10, pady=10)
    Entry(signup_window, textvariable=signup_password_var, show="*").grid(row=1, column=1, padx=10, pady=10)
    
    Label(signup_window, text="Initial Pending Fee").grid(row=2, column=0, padx=10, pady=10)
    Entry(signup_window, textvariable=signup_fee_var).grid(row=2, column=1, padx=10, pady=10)
    
    Button(signup_window, text="Submit", command=submit_signup).grid(row=3, column=0, columnspan=2, pady=10)
...
def main():
    ...
    if conn is not None:
        ...
        Button(root, text="Signup", command=signup).grid(row=3, column=0, columnspan=2, pady=10)
